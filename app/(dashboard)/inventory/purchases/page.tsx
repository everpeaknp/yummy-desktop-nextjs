...
import { Plus, Loader2, MoreVertical, Ban, Undo2 } from "lucide-react";
...
export default function InventoryPurchasesPage() {
  ...
  const [createLines, setCreateLines] = useState<PurchaseLineDraft[]>([
    newPurchaseLineDraft(),
  ]);
  ...
  const openPurchaseDetail = useCallback(
    async (purchase: any) => {
      ...
      try {
        const [purchaseResponse, statementResponse, returnsResponse] =
          await Promise.all([
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    const lines = createLines.filter((l) =>
      l.mode === "existing"
        ...
        : l.newItem.name.trim() && l.newItem.unit.trim(),
    );
    if (lines.length === 0) {
      ...
      const payload = {
        ...
        paid_amount:
          createPaymentStatus === "partial"
            ? Number(createPaidAmount)
            : undefined,
        account_type: ["paid", "partial"].includes(createPaymentStatus)
          ? createAccount?.account_type
          : undefined,
        account_id: ["paid", "partial"].includes(createPaymentStatus)
          ? createAccount?.id
          : undefined,
        idempotency_key: crypto.randomUUID(),
        lines: lines.map((line) => ({
          inventory_item_id:
            line.mode === "existing" ? line.inventoryItemId : undefined,
          ...
          new_item:
            line.mode === "new"
              ? {
                  ...
                  storage_location:
                    line.newItem.storage_location.trim() || undefined,
                }
              : undefined,
  ...
  const handleVoid = async () => {
    ...
    try {
      await apiClient.post(
        PurchaseApis.void(voidPurchase.id, user.restaurant_id),
    ...
    <AppPage width="wide" className="p-4 pb-24 sm:p-6">
      ...
      <PageSection surface className="space-y-3">
        ...
        <p className="text-xs leading-5 text-muted-foreground">
          A posted purchase changes stock only by the quantity actually
          received.
        </p>
      </PageSection>
              ...
              <div key={purchase.id} className="p-4">
                ...
                >
                  ...
                  <div className="mt-2 truncate text-xs text-muted-foreground">
                    {purchase.reference_number || "No supplier reference"}
                  </div>
                </button>
          ...
          <Table className="min-w-[820px] table-fixed">
            <TableHeader>
              <TableRow>
                ...
                <TableHead className="w-[28%] min-w-0">Supplier</TableHead>
                <TableHead className="w-32 whitespace-nowrap">
                  Purchase
                </TableHead>
                <TableHead className="w-[24%] min-w-0">
                  Supplier reference
                </TableHead>
                <TableHead className="w-32 whitespace-nowrap">Total</TableHead>
        ...
        <DialogContent className="flex h-[calc(100dvh-1rem)] w-[calc(100vw-1rem)] max-w-[720px] flex-col gap-0 overflow-hidden p-0 sm:h-auto sm:max-h-[92vh] sm:w-full">
          ...
          >
            <DialogHeader className="shrink-0 border-b px-5 py-4 text-left sm:px-6">
              ...
              <DialogDescription>
                Add the supplier bill and items once. Saving immediately updates
                stock, supplier balance, and accounting.
              </DialogDescription>
            </DialogHeader>
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <div className="space-y-2">
                  ...
                  <Select
                    ...
                    onValueChange={(v) =>
                      setCreateForm({ ...createForm, supplier_id: v })
                    }
                  >
                    <SelectTrigger>
                  ...
                  {createPaymentStatus === "partial" ? (
                    <div className="space-y-2">
                      ...
                      <p className="text-xs text-muted-foreground">
                        The remaining amount stays as a supplier bill.
                      </p>
                    </div>
                  ) : null}
                </div>
      ...
      >
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            <DialogTitle>
              Void {voidPurchase?.reference_number || "supplier purchase"}
            </DialogTitle>
            <DialogDescription>
              This reverses the received stock and reverses the linked expense.
              Use a Purchase Return instead if the goods already left the
              restaurant.
            </DialogDescription>
          </DialogHeader>
