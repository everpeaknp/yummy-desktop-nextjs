...
const categories = [
  {
    ...
    items: [
      ...
      legacySettingItems.switch_restaurant,
    ],
  },
  {
    title: "Business",
    items: [
      ...
      legacySettingItems.branding,
      legacySettingItems.gallery_management,
      {
        id: "restaurant_operations",
        title: "Restaurant operations",
        description: "POS and kitchen operational settings",
        href: "/manage/settings?tab=advanced",
        icon: Settings,
      },
    ],
  },
  {
    ...
    items: [
      legacySettingItems.printer_management,
      {
        ...legacySettingItems.receipt_designer,
        href: "/manage/receipt-designer",
      },
      { ...legacySettingItems.kot_designer, href: "/manage/kot-designer" },
    ],
  },
  {
    title: "Notifications",
    items: [
      legacySettingItems.kitchen_sounds,
      legacySettingItems.push_alerts,
      legacySettingItems.email_summaries,
      legacySettingItems.kot_notifications,
      legacySettingItems.order_notifications,
    ],
  },
  {
    title: "People & access",
    items: [
      ...
      legacySettingItems.admin_management,
    ],
  },
  {
    title: "Finance & compliance",
    items: [
      ...
      { ...legacySettingItems.tax_configuration, href: "/manage/taxes" },
      {
        id: "payment_integrations",
        title: "Payment integrations",
        description: "Payment provider and QR configuration",
        href: "/manage/settings?tab=payments",
        icon: Percent,
      },
    ],
  },
  {
    ...
    items: [
      { ...legacySettingItems.audit_logs, href: "/manage/audit-logs" },
      legacySettingItems.data_export,
      legacySettingItems.auto_backup,
    ],
  },
  {
    title: "Billing",
  ...
  },
  {
    ...
    items: [legacySettingItems.delete_account],
  },
];
...
export default function AdditionalSettingsPage() {
  ...
  const [selectingFor, setSelectingFor] = useState<"profile" | "cover" | null>(
    null,
  );
  ...
  const handleGallerySelect = async (item: MenuGalleryItem) => {
    ...
    try {
      ...
      if (response.data.status === "success") {
        await fetchRestaurant(true);
        toast.success(
          `${selectingFor === "profile" ? "Profile picture" : "Cover photo"} updated`,
        );
    ...
    switch (selectedSetting) {
      ...
      case "auto_backup":
        ...
        const currentItem = categories
          ...
          .find((i: any) => i.id === selectedSetting);
                  ...
                  <Button
                    ...
                    onClick={() => {
                      ...
                      audio
                        ...
                        .catch((e) =>
                          toast.error(
                            "Browser blocked audio. Click to enable.",
                          ),
                        );
                    }}
      ...
      case "admin_management":
        return restaurant ? (
          <AdminManagement restaurantId={restaurant.id} />
        ) : null;
      ...
      case "branding":
        return (
          <div className="space-y-8 py-2">
            <div className="space-y-1">
              <h3 className="text-lg font-bold tracking-tight uppercase italic underline decoration-primary/30 decoration-4 underline-offset-4">
                Restaurant Branding
              </h3>
              <p className="text-sm text-muted-foreground font-bold opacity-60">
                Customize how your restaurant appears across the platform.
              </p>
            </div>
              ...
              <div className="space-y-4">
                ...
                <div className="relative group w-32 h-32 rounded-3xl overflow-hidden border-2 border-dashed border-border/60 bg-muted/30 flex items-center justify-center">
                  {restaurant?.profile_picture ? (
                    <img
                      src={
                        restaurant.profile_picture.startsWith("http")
                          ? restaurant.profile_picture
                          : `/${restaurant.profile_picture.replace("asset:", "assets/")}`
                      }
                      alt="Logo"
                ...
                <div className="space-y-1">
                  <p className="text-[10px] text-muted-foreground font-black uppercase tracking-tight italic opacity-60">
                    1:1 square ratio, min 400x400px
                  </p>
                  <p className="text-[11px] text-foreground font-bold">
                    Appears in customer search and invoices.
                  </p>
                </div>
              </div>
              ...
              <div className="space-y-4">
                ...
                <div className="relative group h-32 rounded-3xl overflow-hidden border-2 border-dashed border-border/60 bg-muted/30 flex items-center justify-center">
                  {restaurant?.cover_photo ? (
                    <img
                      src={
                        restaurant.cover_photo.startsWith("http")
                          ? restaurant.cover_photo
                          : `/${restaurant.cover_photo.replace("asset:", "assets/")}`
                      }
                      alt="Cover"
                      className="w-full h-full object-cover"
                    />
                ...
                <div className="space-y-1">
                  <p className="text-[10px] text-muted-foreground font-black uppercase tracking-tight italic opacity-60">
                    16:9 landscape ratio, min 1200x675px
                  </p>
                  <p className="text-[11px] text-foreground font-bold">
                    Appears as your restaurant header background.
                  </p>
                </div>
              </div>
            </div>
  ...
  return (
    <AppPage width="wide" className="pb-24 lg:pb-8">
      <PageHeader
      ...
      <Dialog
        ...
        onOpenChange={(open: boolean) => !open && setSelectedSetting(null)}
      >
        <DialogContent
          className={cn(
          ...
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2 text-2xl font-black tracking-tight uppercase italic">
              {selectedSetting &&
                categories
                  .flatMap((c: any) => c.items)
                  .find((i: any) => i.id === selectedSetting)?.title}
            </DialogTitle>
            <DialogDescription className="font-bold text-muted-foreground/80">
              Configure your {selectedSetting?.replace("_", " ")} preferences
      ...
      {filteredCategories.length === 0 && (
        <div className="flex flex-col items-center justify-center py-20 text-center space-y-4">
          ...
          <div>
            <p className="text-lg font-black tracking-tight italic uppercase">
              No settings match your request
            </p>
            <p className="text-muted-foreground font-bold opacity-60">
              Try searching for alternative keywords.
            </p>
          </div>
        </div>
      )}
