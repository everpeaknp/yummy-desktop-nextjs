...
export default function RestaurantSettingsPage() {
  ...
  const loadBanks = useCallback(async () => {
    ...
    try {
      const res = await apiClient.get(
        AccountingApis.paymentBanks(user.restaurant_id),
      );
  ...
  const handleCardDelete = async (index: number) => {
    if (!user?.restaurant_id || !Array.isArray(restaurant?.payment_cards))
      return;
        ...
        <TabsContent value="payments" className="space-y-6">
          <Card className="border-primary/20 bg-primary/5">
            <CardHeader>
              ...
              <CardDescription>
                Bank and custom accounts, checkout payment methods, and cash
                drawers are financial controls. Manage them from Finance so
                balances, custody, and settlement stay together.
              </CardDescription>
            </CardHeader>
            <CardContent className="flex flex-wrap gap-2">
              <Button asChild>
                <Link href="/finance/operations">Cash &amp; Banks</Link>
              </Button>
              <Button asChild variant="outline">
                <Link href="/finance/operations">Payment Instruments</Link>
              </Button>
              <Button asChild variant="outline">
                <Link href="/cash-drawers">Cash Drawers</Link>
              </Button>
            </CardContent>
          </Card>
                    ...
                    >
                      ...
                      {isReviewBank(bank) ? (
                        <div className="mt-2 text-xs text-amber-600">
                          Review this fallback bank and reassign instruments to
                          the real bank when ready.
                        </div>
                      ) : null}
                    </div>
                  ))}
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Set up automated QR payments for your POS. Then connect one
                FonePay Dynamic QR instrument to its settlement account in{" "}
                <Link className="underline" href="/finance/operations">
                ...
                <div className="flex items-center justify-between p-4 bg-orange-50 rounded-lg border border-orange-100 mb-4">
                  <div className="space-y-0.5">
                    ...
                    <p className="text-xs text-orange-700/70">
                      Allow customers to pay via QR at checkout.
                    </p>
                  </div>
                  <Switch
                    checked={fonepay.is_active}
                    onCheckedChange={(val) =>
                      setFonepay({ ...fonepay, is_active: val })
                    }
                  />
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setFonepay({ ...fonepay, api_user: e.target.value })
                      }
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="relative">
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          setFonepay({ ...fonepay, api_secret: e.target.value })
                        }
                        placeholder="••••••••••••"
                      />
                    </div>
          ...
          <Card className="hidden" aria-hidden="true">
            <CardHeader className="flex flex-row items-center justify-between space-y-0">
              <div className="space-y-1">
                ...
                <CardDescription>
                  Add your card accounts/terminals to record exactly which card
                  received payment.
                </CardDescription>
              </div>
              <Button
        ...
        <TabsContent value="advanced">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Critical flags that change POS behavior.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="flex items-center justify-between p-4 border rounded-lg bg-blue-50/50 border-blue-100">
                <div className="space-y-0.5">
                  ...
                  <p className="text-xs text-muted-foreground mr-8">
                    When disabled, KOTs will be auto-printed instead of shown
                    digitally. Kitchen screen will be hidden for all staff.
                  </p>
                </div>
                <Switch
              ...
              <div className="flex items-center justify-between p-4 border rounded-lg">
                <div className="space-y-0.5">
                  <Label className="flex items-center gap-2">
                    Enable Tax (VAT)
                    {isActiveVat && (
                      <Badge variant="success">IRD managed</Badge>
                    )}
                  </Label>
                  <p className="text-xs text-muted-foreground mr-8">
                    {isActiveVat
                ...
                <Switch
                  checked={
                    isActiveVat ? true : (restaurant?.tax_enabled ?? false)
                  }
                  ...
                  onCheckedChange={async (val) => {
                    ...
                    try {
                      ...
                      toast.success(
                        `Tax calculation ${val ? "enabled" : "disabled"}`,
                      );
                    } catch (err) {
              ...
              <div className="flex items-center justify-between p-4 border rounded-lg opacity-50">
                <div className="space-y-0.5">
                  ...
                  <p className="text-xs text-muted-foreground italic">
                    Restricted: Service charges are not recommended.
                  </p>
                </div>
              ...
              <div className="flex items-center justify-between p-4 border rounded-lg">
                <div className="space-y-0.5">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Prevent sales if stock is zero.
                  </p>
                </div>
      ...
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              {qrDialog.index !== null
                ? "Edit QR Configuration"
                : "Add New static QR"}
            </DialogTitle>
            <DialogDescription>
              Enter the display name and actual text-payload for the merchant
              QR.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4 py-4">
            ...
            <div className="space-y-2">
              ...
              <Textarea
                ...
                onChange={(e) =>
                  setQrForm({ ...qrForm, payload: e.target.value })
                }
            ...
            <div className="space-y-2">
              ...
              <Select
                value={String(qrForm.bank_id || "none")}
                onValueChange={(val) => setQrForm({ ...qrForm, bank_id: val })}
              >
                <SelectTrigger>
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={saving || !qrForm.name || !qrForm.payload}
            >
      ...
      >
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Set a clear card name and optional identifier (e.g., last 4 digits
              or terminal code).
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4 py-4">
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) =>
                  setCardForm({ ...cardForm, identifier: e.target.value })
                }
            ...
            <div className="space-y-2">
              <Label>Bank Assignment (Optional)</Label>
              <Select
                value={String(cardForm.bank_id || "none")}
                onValueChange={(val) =>
                  setCardForm({ ...cardForm, bank_id: val })
                }
              >
                <SelectTrigger>
                  <SelectValue placeholder="Select a bank" />
