...
import {
  humanizeKotEvent,
  isKotDelayed,
  KotCompactTicketCard,
  KotTicketHeader,
} from "@/components/kitchen/kot-ticket-card";
...
interface OrdersKotItem {
  ...
  qty_served?: number;
  notes?: string;
  modifiers?: { id: number | string; modifier_name_snapshot: string }[];
  is_deleted?: number;
}
...
function orderMatchesDetailFilters(
  order: any,
  filters: OrderDetailFilters,
  staffField: "created" | "completed" = "created",
) {
  const normalized = (value: unknown) =>
    String(value || "")
      .trim()
      .toLowerCase();
  if (filters.status && normalized(order.status) !== normalized(filters.status))
    return false;
  if (
    filters.channel &&
    normalized(order.channel) !== normalized(filters.channel)
  )
    return false;
  if (
    filters.tableCategory &&
    normalized(order.table_category_name || order.table_category) !==
      normalized(filters.tableCategory)
  )
    return false;
  if (
    filters.tableName &&
    normalized(order.table_name) !== normalized(filters.tableName)
  )
    return false;
  const staffName =
    staffField === "completed"
  ...
  if (
    filters.createdBy &&
    normalized(staffName) !== normalized(filters.createdBy)
  )
    return false;
...
function matchesOrderTimeScope(
  value: string | undefined,
  scope: OrderTimeScope,
) {
  ...
  if (scope === "last7")
    return (
      date >= startOfDay(subDays(new Date(), 6)) && date <= endOfDay(new Date())
    );
  return (
    date >=
      startOfDay(
...
function TimeScopeChips({
  value,
  onChange,
}: {
  value: OrderTimeScope;
  onChange: (scope: OrderTimeScope) => void;
}) {
  return (
...
function OrderDetailFilterFields({
  ...
  const options = (read: (order: any) => unknown) =>
    Array.from(
      new Set(
        orders
          .map(read)
  ...
  const tableCategories = options(
    (order) => order.table_category_name || order.table_category,
  );
  const tables = options((order) => order.table_name).filter((table) => {
    if (!filters.tableCategory) return true;
    return orders.some(
      (order) =>
        String(order.table_name || "").trim() === table &&
        String(order.table_category_name || order.table_category || "")
          .trim()
          .toLowerCase() === filters.tableCategory.toLowerCase(),
    );
  });
    ...
    <div className="space-y-4">
      <ChoiceChips
        ...
        options={statuses.map((status) => ({
          value: status,
          label: readableFilterValue(status),
        }))}
      />
      <ChoiceChips
        label="Order type"
        ...
        options={channels.map((channel) => ({
          value: channel,
          label: readableFilterValue(channel),
        }))}
      />
      <ChoiceChips
        label="Table area"
        value={filters.tableCategory}
        onChange={(tableCategory) => onChange({ tableCategory, tableName: "" })}
        options={tableCategories.map((category) => ({
          value: category,
          label: category,
        }))}
      />
      <ChoiceChips
        ...
        onChange={(tableName) => onChange({ tableName })}
      ...
      <div className="grid gap-2">
        ...
        <Select
          ...
          onValueChange={(value) =>
            onChange({ createdBy: value === "all" ? "" : value })
          }
        >
          ...
          <SelectContent className="max-h-64 rounded-xl border-border p-1 shadow-lg">
            <SelectItem value="all" className="rounded-lg py-2.5 text-sm">
              All staff
            </SelectItem>
            {users.map((user) => (
              <SelectItem
      ...
      <div>
        ...
        <div className="mt-2 grid grid-cols-2 gap-3">
          <Input
            ...
            onChange={(event) => onChange({ minTotal: event.target.value })}
          ...
          <Input
            ...
            onChange={(event) => onChange({ maxTotal: event.target.value })}
            className="h-11 rounded-xl"
          />
        </div>
      </div>
...
export default function OrdersPage() {
  const [activeTab, setActiveTab] = useState<"active" | "kot" | "history">(
    "active",
  );
  ...
  const [activeDetailFilters, setActiveDetailFilters] =
    useState<OrderDetailFilters>(emptyOrderDetailFilters);
  const [historyDetailFilters, setHistoryDetailFilters] =
    useState<OrderDetailFilters>(emptyOrderDetailFilters);
  ...
  const fetchActiveData = useCallback(async () => {
    ...
    try {
      ...
      if (ordersRes.data.status === "success") {
        const fetchedOrders = [...(ordersRes.data.data.orders || [])].sort(
      ...
      console.error(
        "Failed to fetch active data response:",
        (err as any)?.response?.data,
      );
      console.error(
        "Failed to fetch active data response json:",
  ...
  const fetchKotData = useCallback(async () => {
    ...
    try {
      ...
      const res = await apiClient.get(
        `${KotApis.searchKots}?${params.toString()}`,
      );
    ...
    try {
      ...
      if (res.data.status === "success") {
        ...
        try {
          ...
          console.warn(
            "Order settlement summaries are unavailable",
            settlementError,
          );
          setHistorySettlements({});
        }
  ...
  useEffect(() => {
    ...
    apiClient
      .get(KotApis.getKotActivity(selectedKot.id), {
        params: { skip: 0, limit: 100 },
      })
      .then((res) => {
        if (cancelled) return;
        setKotActivity(
          res.data?.status === "success" ? res.data?.data?.items || [] : [],
        );
      })
      .catch(() => {
  ...
  const updateKotStatus = useCallback(
    async (kotId: number, newStatus: OrdersKotStatus) => {
      if (kotStatusUpdating) return;
      setKotStatusUpdating(true);
      try {
        await apiClient.patch(KotApis.updateKotStatus(kotId), {
          status: newStatus,
        });
        setSelectedKot((current) =>
          current && current.id === kotId
        ...
        toast.error(
          typeof detail === "string" ? detail : "Failed to update KOT status",
        );
      } finally {
  ...
  const rejectKot = useCallback(
    async (kotId: number) => {
      if (kotStatusUpdating || !window.confirm("Reject this kitchen ticket?"))
        return;
      setKotStatusUpdating(true);
      try {
        ...
        setKots((current) =>
          current.map((kot) =>
            kot.id === kotId ? { ...kot, status: "REJECTED" } : kot,
          ),
        );
        ...
        const detail =
          error?.response?.data?.detail || error?.response?.data?.message;
        toast.error(
          typeof detail === "string" ? detail : "Failed to reject KOT",
        );
      } finally {
        setKotStatusUpdating(false);
      }
  ...
  const filteredHistoryOrders = useMemo(
    () =>
      historyOrders.filter((order) =>
        orderMatchesDetailFilters(order, historyDetailFilters, "completed"),
      ),
  ...
  const filteredKots = useMemo(() => {
    ...
    return kots
      ...
      .filter(
        (kot) =>
          kotStatusFilter === "ALL" ||
          String(kot.status).toUpperCase() === kotStatusFilter,
      )
      .filter(
        (kot) =>
          kotStationFilter === "All" ||
          (kot.station || "").toLowerCase() === kotStationFilter.toLowerCase(),
      )
      .filter(
        (kot) =>
          kotTableFilter === "All" ||
          String(kot.table_name || "").toLowerCase() ===
            kotTableFilter.toLowerCase(),
      )
      .filter((kot) => {
    ...
    activeTab === "active" ? (
      <>
        ...
        <OrderDetailFilterFields
          ...
          onChange={(next) =>
            setActiveDetailFilters((current) => ({ ...current, ...next }))
          }
        />
      </>
    ) : activeTab === "kot" ? (
      <>
        ...
        <ChoiceChips
          ...
          value={kotStatusFilter === "ALL" ? "" : kotStatusFilter}
          onChange={(status) =>
            setKotStatusFilter((status || "ALL") as OrdersKotStatus | "ALL")
          }
          options={(
            ["PENDING", "PREPARING", "READY", "SERVED", "REJECTED"] as const
          ).map((status) => ({ value: status, label: kotStatusLabel(status) }))}
        />
        <ChoiceChips
          label="Station"
          value={kotStationFilter === "All" ? "" : kotStationFilter}
          onChange={(station) => setKotStationFilter(station || "All")}
          options={Array.from(
            new Set(kots.map((kot) => kot.station).filter(Boolean) as string[]),
          ).map((station) => ({ value: station, label: station }))}
        />
        <ChoiceChips
          label="Table"
          value={kotTableFilter === "All" ? "" : kotTableFilter}
          onChange={(table) => setKotTableFilter(table || "All")}
          options={Array.from(
            new Set(
              kots.map((kot) => kot.table_name).filter(Boolean) as string[],
            ),
  ...
  return (
    <AppPage
      width="wide"
      className="pb-[calc(9rem+env(safe-area-inset-bottom))] lg:pb-10"
    >
      <PageHeader
        title="Orders"
        description={
          activeTab === "history"
            ? "Completed and historical orders for review."
            : "Live orders, completed sales, and their operational context."
        }
        actions={
              ...
              mobileFooter={
                activeTab === "active" ? (
                  <Button
                    ...
                    className="w-full"
                    onClick={() => {
                      setActiveTimeScope("all");
                      setActiveDetailFilters(emptyOrderDetailFilters);
                    }}
                  >
                    Clear filters
                  </Button>
                  ...
                  <Button
                    ...
                    className="w-full"
                    onClick={() => {
                      setKotTimeScope("today");
                      setKotStatusFilter("ALL");
                      setKotStationFilter("All");
                      setKotTableFilter("All");
                    }}
                  >
                    Clear filters
                  </Button>
        ...
        <div className="min-w-0">
          ...
          {activeTab === "kot" && (
            <FilterBar
              ...
              title="KOT filters"
              activeCount={
                (kotStatusFilter === "ALL" ? 0 : 1) +
                (kotStationFilter === "All" ? 0 : 1) +
                (kotTableFilter === "All" ? 0 : 1)
              }
            >
              {(
                [
          ...
          {activeTab === "active" ? (
            loading ? (
              ...
              <SharedEmptyState
                ...
                description="Try changing the active-order filter or search."
                icon={<ClipboardList className="h-5 w-5" />}
              />
            ) : (
              <>
                <div className="mb-3 flex items-center gap-2 text-sm font-semibold sm:mb-4">
            ...
            kotLoading && kots.length === 0 ? (
              ...
              <SharedEmptyState
                ...
                description="Try changing the KOT filters or search."
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-4 lg:gap-5">
                    {orders.map((order) => (
                      ...
                      >
                        <OrderHistoryCard
                          order={order}
                          settlement={historySettlements[order.id]}
                        />
                      </Link>
                    ))}
                  </div>
      ...
      >
        <DialogContent className="flex h-[100dvh] w-full max-w-none min-w-0 flex-col overflow-hidden rounded-none p-0 sm:h-auto sm:max-h-[90vh] sm:w-[calc(100vw-3rem)] sm:max-w-[760px] sm:rounded-2xl">
          {selectedKot ? (
            <DialogHeader className="min-w-0 border-b border-border/50 bg-muted/20 p-4 pr-12 text-left sm:p-6 sm:pr-14">
              <KotTicketHeader
                ticket={selectedKot}
                delayed={isKotDelayed(selectedKot)}
              />
              ...
              {nextKotStatus(selectedKot.status) ? (
                <div className="mt-3 flex min-w-0 flex-col gap-2 sm:flex-row sm:items-center">
                  ...
                  <Button
                    type="button"
                    variant="outline"
                    className="h-11 w-full border-destructive/30 text-destructive hover:bg-destructive/10 hover:text-destructive sm:w-auto"
                    disabled={kotStatusUpdating}
            ...
            <div className="min-h-0 flex-1 space-y-5 overflow-y-auto overscroll-contain p-4 sm:p-6">
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
                ...
                <div className="rounded-xl border border-border/50 bg-card p-4">
                  ...
                  <p className="mt-1 truncate text-sm font-bold">
                    {selectedKot.created_by_staff_name || "System"}
                  </p>
                </div>
                <div className="rounded-xl border border-border/50 bg-card p-4">
                  <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">
                  ...
                  <Link
                    href={`/orders/${selectedKot.order_id}`}
                ...
                <div className="flex min-w-0 flex-wrap items-center justify-between gap-2 border-b border-border/50 px-4 py-3">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      Everything recorded on this kitchen ticket.
                    </p>
                  </div>
                  <Badge variant="secondary" className="rounded-lg text-xs">
                    {selectedKot.items?.filter(
                      (item) =>
                        !item.is_deleted && Math.abs(item.qty_change || 0) > 0,
                    ).length || 0}{" "}
                    items
                  </Badge>
                </div>
                  ...
                  {(selectedKot.items || [])
                    ...
                    .map((item) => {
                      ...
                      const ready = Math.min(
                        Math.abs(item.qty_ready || 0),
                        quantity,
                      );
                      return (
                        <div
                          key={item.id}
                          className="flex min-w-0 flex-col gap-2 px-4 py-3 sm:flex-row sm:items-start sm:justify-between sm:gap-4"
                        >
                          <div className="min-w-0 flex-1">
                            ...
                            {item.notes ? (
                              <p className="mt-2 break-words border-l-2 border-amber-500/70 pl-2 text-xs font-medium text-foreground">
                          ...
                          <div className="flex min-w-0 flex-wrap items-center gap-2 text-xs font-semibold text-muted-foreground sm:shrink-0 sm:justify-end">
                            <span
                              className={cn(
              ...
              <section className="rounded-xl border border-border/50 bg-card">
                <div className="border-b border-border/50 px-4 py-3">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Status and preparation changes for this ticket.
                  </p>
                </div>
                <div className="divide-y divide-border/40">
                  {kotActivityLoading ? (
                      ...
                      >
                        <div className="min-w-0">
                          <p className="break-words text-sm font-semibold">
                            {humanizeKotEvent(activity.event)}
                          </p>
                        ...
                        <time className="text-xs text-muted-foreground sm:shrink-0 sm:whitespace-nowrap">
                          {activity.created_at
...
function KotOrderCard({
  kot,
  onClick,
}: {
  kot: OrdersKot;
  onClick: (kot: OrdersKot) => void;
}) {
  return <KotCompactTicketCard kot={kot} onOpenDetails={() => onClick(kot)} />;
}
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
      {Array.from({ length: 8 }).map((_, i) => (
        <div
          key={i}
          className="h-40 rounded-2xl bg-muted/40 animate-pulse border border-border/40"
        />
      ))}
    </div>
  );
