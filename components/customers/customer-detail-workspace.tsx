  ...
  return <div className="mx-auto max-w-[1440px] space-y-6 p-4 md:p-6">
    ...
    <div className="grid gap-4 lg:grid-cols-[300px_1fr]">
      ...
      <div className="space-y-4"><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3"><Metric label="Open invoices" value={formatCurrency(receivable)} icon={<WalletCards className="h-4 w-4" />} /><Metric label="Customer credit" value={formatCurrency(customerCredit)} icon={<Landmark className="h-4 w-4" />} /><Metric label="Sales" value={formatCurrency(totalSales)} icon={<ClipboardList className="h-4 w-4" />} /><Metric label="Sales returns" value={formatCurrency(totalReturns)} icon={<RotateCcw className="h-4 w-4" />} /><Metric label="Payments received" value={formatCurrency(totalCollected)} icon={<ReceiptText className="h-4 w-4" />} /><Metric label="Payments out" value={formatCurrency(totalPaidOut)} icon={<Banknote className="h-4 w-4" />} /></div>
        ...
        {tab === "activity" && <ActivityList entries={entries} returns={returns} />}
        {tab === "sales" && <InvoiceList invoices={invoices} openInvoices={openInvoices} onReturn={(invoiceId) => { setReturnOpen(true); }} />}
        {tab === "returns" && <ReturnList returns={returns} />}
        {tab === "payments" && <PaymentList entries={[...collections, ...customerPaymentsOut]} returns={returns} />}
        {tab === "open" && <OpenInvoiceList invoices={openInvoices} onCollect={openCollection} onReturn={() => setReturnOpen(true)} />}
        {tab === "statement" && <StatementWithAllocations entries={entries} allocations={statement?.allocations || []} />}
      </div>
    </div>
...
function ActivityList({ entries, returns }: { entries: any[]; returns: FinanceSalesDocument[] }) { const refundReturns = returns.filter((item) => item.settlement_status === "refunded" || item.settlement_status === "refund_now"); return <Card><CardHeader className="pb-3"><CardTitle className="text-base">Activity</CardTitle><p className="text-xs text-muted-foreground">Amounts describe what happened, rather than accounting debit and credit signs.</p></CardHeader><CardContent className="space-y-1">{[...entries.map((entry) => ({ type: "entry" as const, at: entry.occurred_at || entry.created_at, data: entry })), ...refundReturns.map((document) => ({ type: "refund" as const, at: document.created_at, data: document }))].sort((a, b) => String(b.at).localeCompare(String(a.at))).map((item) => { if (item.type === "refund") return <div key={`refund-${item.data.id}`} className="flex items-center justify-between gap-4 rounded-md px-2 py-3 hover:bg-muted/50"><div><p className="text-sm font-medium">Sales return {item.data.document_number}</p><p className="text-xs text-muted-foreground">{formatDate(item.data.business_date)} · Refund paid to customer</p></div><p className="text-sm font-semibold text-orange-600">Refunded {formatCurrency(item.data.grand_total)}</p></div>; const presentation = customerActivity(item.data); return <div key={item.data.id} className="flex items-center justify-between gap-4 rounded-md px-2 py-3 hover:bg-muted/50"><div><p className="text-sm font-medium">{item.data.display_name}</p><p className="text-xs text-muted-foreground">{formatDate(item.data.financial_date)} · {item.data.source_reference || item.data.entry_type.replaceAll("_", " ")}</p></div><div className="text-right"><p className={`text-sm font-semibold ${presentation.class}`}>{presentation.label} {formatCurrency(item.data.amount)}</p><p className="text-xs text-muted-foreground">{customerOpenLabel(item.data)}</p></div></div>; })}</CardContent></Card>; }
function InvoiceList({ invoices, openInvoices, onReturn }: { invoices: FinanceSalesDocument[]; openInvoices: any[]; onReturn: (invoiceId: number) => void }) { const openByInvoice = new Map(openInvoices.filter((entry) => entry.source_type === "finance_sales_invoice").map((entry) => [entry.source_id, entry])); return <Card><CardHeader className="pb-3"><CardTitle className="text-base">Sales invoices</CardTitle></CardHeader><CardContent className="space-y-2">{invoices.length ? invoices.map((invoice) => { const entry = openByInvoice.get(invoice.id); return <div key={invoice.id} className="flex flex-col gap-3 rounded-lg border p-3 sm:flex-row sm:items-center sm:justify-between"><div><p className="font-medium">{invoice.document_number}</p><p className="text-xs text-muted-foreground">{formatDate(invoice.business_date)} · {invoice.source_type === "pos_order" ? "POS sale" : "Manual sale"}</p></div><div className="flex items-center gap-3"><div className="text-right"><p className="font-medium">{formatCurrency(invoice.grand_total)}</p><p className="text-xs text-muted-foreground">{entry ? `Remaining ${formatCurrency(entry.open_amount)}` : "Settled"}</p></div><Button variant="outline" size="sm" onClick={() => onReturn(invoice.id)}>Return</Button></div></div>; }) : <Empty text="No sales invoices have been recorded for this customer." />}</CardContent></Card>; }
function ReturnList({ returns }: { returns: FinanceSalesDocument[] }) { return <Card><CardHeader className="pb-3"><CardTitle className="text-base">Sales returns</CardTitle></CardHeader><CardContent className="space-y-2">{returns.length ? returns.map((creditNote) => <div key={creditNote.id} className="flex items-center justify-between gap-4 rounded-lg border p-3"><div><p className="font-medium">{creditNote.document_number}</p><p className="text-xs text-muted-foreground">{formatDate(creditNote.business_date)} · {creditNote.settlement_status.replaceAll("_", " ")}</p></div><p className="font-medium text-emerald-600">{formatCurrency(creditNote.grand_total)}</p></div>) : <Empty text="No sales returns have been recorded for this customer." />}</CardContent></Card>; }
function PaymentList({ entries, returns }: { entries: any[]; returns: FinanceSalesDocument[] }) { const refunds = returns.filter((item) => item.settlement_status === "refunded" || item.settlement_status === "refund_now"); return <Card><CardHeader className="pb-3"><CardTitle className="text-base">Payments in and out</CardTitle></CardHeader><CardContent className="space-y-2">{entries.length || refunds.length ? <>{entries.map((entry) => { const paymentOut = entry.entry_type === "customer_payment_out"; return <div key={entry.id} className="flex items-center justify-between gap-4 rounded-lg border p-3"><div><p className="font-medium">{entry.display_name}</p><p className="text-xs text-muted-foreground">{formatDate(entry.financial_date)} · {entry.payment_method?.replaceAll("_", " ") || "Payment method not recorded"}</p></div><div className="text-right"><Badge variant="outline">{paymentOut ? "Payment out" : "Payment in"}</Badge><p className={`mt-1 font-medium ${paymentOut ? "text-orange-600" : "text-emerald-600"}`}>{paymentOut ? "Paid" : "Received"} {formatCurrency(entry.amount)}</p></div></div>; })}{refunds.map((creditNote) => <div key={`refund-${creditNote.id}`} className="flex items-center justify-between gap-4 rounded-lg border p-3"><div><p className="font-medium">Refund for {creditNote.document_number}</p><p className="text-xs text-muted-foreground">{formatDate(creditNote.business_date)} · Sales return</p></div><div className="text-right"><Badge variant="outline">Payment out</Badge><p className="mt-1 font-medium text-orange-600">Refunded {formatCurrency(creditNote.grand_total)}</p></div></div>)}</> : <Empty text="No customer payments or refunds have been recorded." />}</CardContent></Card>; }
function OpenInvoiceList({ invoices, onCollect, onReturn }: { invoices: any[]; onCollect: () => void; onReturn: (invoiceId: number) => void }) { return <Card><CardHeader className="pb-3"><CardTitle className="text-base">Open invoices ready for collection</CardTitle></CardHeader><CardContent className="space-y-2">{invoices.length ? invoices.map((invoice) => <div key={invoice.id} className="flex flex-col gap-3 rounded-lg border p-3 sm:flex-row sm:items-center sm:justify-between"><div><p className="font-medium">{invoice.source_reference || invoice.display_name}</p><p className="text-xs text-muted-foreground">{formatDate(invoice.financial_date)} · Original {formatCurrency(invoice.amount)} · Remaining {formatCurrency(invoice.open_amount)}</p></div><div className="flex gap-2"><Button variant="outline" size="sm" onClick={() => onReturn(invoice.source_id)}>Return</Button><Button size="sm" onClick={onCollect}>Receive payment</Button></div></div>) : <Empty text="There are no unpaid sales invoices." />}</CardContent></Card>; }
...
function StatementWithAllocations({ entries, allocations }: { entries: any[]; allocations: any[] }) { return <div className="space-y-4"><Card><CardHeader className="pb-3"><CardTitle className="text-base">Open-item statement</CardTitle></CardHeader><CardContent className="space-y-2">{entries.length ? entries.map((entry) => <div key={entry.id} className="grid grid-cols-[1fr_auto_auto] items-center gap-4 rounded-lg border p-3"><div><p className="font-medium">{entry.display_name}</p><p className="text-xs text-muted-foreground">{formatDate(entry.financial_date)} · {entry.status}</p></div><Badge variant="outline">{entry.entry_side === "debt" ? "Invoice" : "Payment / credit"}</Badge><div className="text-right text-sm"><p>{formatCurrency(entry.amount)}</p><p className="text-muted-foreground">{customerOpenLabel(entry)}</p></div></div>) : <Empty text="No statement entries have been recorded." />}</CardContent></Card><Card><CardHeader className="pb-3"><CardTitle className="text-base">Settlement allocations</CardTitle></CardHeader><CardContent className="space-y-2">{allocations.length ? allocations.map((allocation) => <div key={allocation.id} className="grid gap-2 rounded-lg border p-3 sm:grid-cols-[1fr_auto_1fr_auto] sm:items-center"><div><p className="text-sm font-medium">{allocation.source_reference || allocation.source_display_name || "Customer payment"}</p><p className="text-xs text-muted-foreground">Payment received</p></div><span className="text-center text-muted-foreground">applied to</span><div><p className="text-sm font-medium">{allocation.target_reference || allocation.target_display_name || "Sales invoice"}</p><p className="text-xs text-muted-foreground">{formatDate(allocation.financial_date)}</p></div><p className="text-right font-semibold">{formatCurrency(allocation.amount)}</p></div>) : <Empty text="No payment allocations have been recorded." />}</CardContent></Card></div>; }
