...
export function MappingExceptionResolver({ report, onCreateMapping }: MappingExceptionResolverProps) {
    ...
    <Card className="border-amber-500/30">
      ...
      <CardContent className="space-y-3 p-4">
        ...
        {rows.map((row) => {
          ...
          return (
            <div
              key={`${row.event_type}-${row.payment_method ?? "any"}-${row.business_line}`}
