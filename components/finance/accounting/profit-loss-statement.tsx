        ...
        <div className="divide-y divide-border border-y border-border">
          {rows.map((row, index) => (
            <div
              ...
              tabIndex={onOpenDrilldown ? 0 : undefined}
              onClick={() =>
                onOpenDrilldown?.(
                  row.account_id,
                  row.account_code
              ...
              className="flex cursor-pointer items-center justify-between px-3 py-2 text-sm transition-colors hover:bg-muted/40 focus-visible:bg-muted/40 focus-visible:outline-none"
            >
              <span>
                {row.account_code
                  ? `${row.account_code} - ${row.account_name}`
