...
import type {
  ...
  FinanceSalesReturnLineInput,
  FinanceSalesReturnSource,
...
export function FinanceSalesReturnDialog({
  ...
  const [sourceType, setSourceType] =
    useState<FinanceSalesReturnSource>("finance_invoice");
  ...
  const [outcome, setOutcome] = useState<"refund_now" | "customer_credit">(
    "refund_now",
  );
  ...
  useEffect(() => {
    ...
    Promise.all([
      financeSalesApi.list(Number(restaurantId), {
        kind: "invoice",
        limit: 200,
      }),
      ...
      .then(
        ([invoiceResult, customerResult, eligibleHeads, initialPosInvoice]) => {
          const availableInvoices =
            initialPosInvoice &&
            !invoiceResult.documents.some(
              (item) => item.id === initialPosInvoice.id,
            )
      ...
      .catch((error) => {
        toast.error(
          error.response?.data?.detail || "Could not load return options.",
        );
      });
  ...
  useEffect(() => {
    ...
    if (invoice.source_type === "pos_order" && invoice.source_id) {
      apiClient
        .get(OrderApis.getOrder(Number(invoice.source_id)))
        .then((response) => {
          ...
          setOriginalPaymentId(
            successful[0]?.id ? String(successful[0].id) : "",
          );
        })
        .catch((error) =>
          toast.error(
            error.response?.data?.detail ||
              "Could not load the original settlement.",
          ),
        );
    } else {
      setOrder(null);
  ...
  const selectedInvoice =
    invoices.find((invoice) => invoice.id === Number(sourceId)) || null;
    ...
    () =>
      selectedRows.reduce((sum, row) => {
        if (sourceType === "external") {
          return (
            sum +
            Number(row.quantity) * Number(row.unit_price || 0) +
        ...
        return (
          sum +
          (row.available
            ? (row.amount * Number(row.quantity)) / row.available
            : 0)
        );
      }, 0),
    [selectedRows, sourceType],
  );
  ...
  const submit = async () => {
    if (!restaurantId || !reason.trim()) {
      toast.error("A return reason is required.");
      return;
    }
    if (sourceType === "finance_invoice" && !sourceId) {
    ...
    if (selectedRows.some((row) => Number(row.quantity) > row.available)) {
      toast.error("Return quantity cannot exceed the sold quantity.");
      return;
    }
    if (
      sourceType === "external" &&
      ...
      toast.error(
        "External returns require a reference, item, positive rate, and sales head.",
      );
      return;
    }
    if (outcome === "refund_now" && !account) {
      toast.error("Choose the account that pays the refund.");
      return;
    }
    if (outcome === "customer_credit" && !customerId) {
      toast.error("Customer credit requires a customer.");
      return;
    }
    ...
    try {
      await financeSalesApi.createReturn({
        ...
        lines: selectedRows.map(
          ({
            ...
            tax_amount:
              line.tax_amount == null ? null : Number(line.tax_amount),
          }),
        ),
        outcome,
        settlement:
          outcome === "refund_now" && account
            ? {
      ...
      toast.error(
        error.response?.data?.detail || "Could not record sales return.",
      );
    } finally {
      setSaving(false);
    }
      ...
      <DialogContent className="max-h-[92vh] max-w-6xl overflow-y-auto p-4 sm:p-6">
        <DialogHeader>
          ...
          <p className="text-sm text-muted-foreground">
            Select the invoice customers recognize. POS and manual sales share
            this register; internal order IDs stay hidden.
          </p>
        </DialogHeader>
                  ...
                  .map((invoice) => (
                    <option key={invoice.id} value={invoice.id}>
                      ...
                      {invoice.daily_order_number
                        ? ` · Daily order #${invoice.daily_order_number}`
                        : ""}{" "}
                      · {invoice.source_type === "pos_order" ? "POS" : "Manual"}{" "}
              ...
              {rows.map((row) => (
                <tr key={row.key} className="border-t">
                  ...
                  <td className="p-3 text-muted-foreground">
                    {sourceType === "external" ? "—" : row.available}
                  </td>
                  <td className="p-2">
                    <Input
                      className="w-28"
                  ...
                  {sourceType === "external" ? (
                    <>
                      ...
                      <td className="p-2">
                        <select
                          className="h-10 min-w-48 rounded-md border bg-background px-2"
                          value={row.reporting_head_id || ""}
                          onChange={(event) =>
                            updateRow(row.key, {
                              reporting_head_id: Number(event.target.value),
        ...
        {sourceType === "external" ? (
          <Button
            ...
            onClick={() =>
              setRows((current) => [
                ...current,
                { ...manualRow(), reporting_head_id: heads[0]?.id || null },
              ])
            }
          >
        ...
        <div className="grid gap-4 rounded-2xl border p-4 lg:grid-cols-[1fr_360px]">
          <div className="space-y-4">
            <div className="space-y-2">
              <Label>
                Customer {outcome === "customer_credit" ? "*" : "(optional)"}
              </Label>
              <select
                className="h-10 w-full rounded-md border bg-background px-3 text-sm"
                value={customerId}
            ...
            successfulPayments.length ? (
              <div className="space-y-2">
                <Label>Original payment to reverse</Label>
                <select
                  className="h-10 w-full rounded-md border bg-background px-3 text-sm"
                  value={originalPaymentId}
                  onChange={(event) => setOriginalPaymentId(event.target.value)}
                >
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 rounded-lg bg-muted p-1">
              <Button
                variant={outcome === "refund_now" ? "default" : "ghost"}
                onClick={() => setOutcome("refund_now")}
              >
                Refund now
              </Button>
              <Button
                variant={outcome === "customer_credit" ? "default" : "ghost"}
                onClick={() => setOutcome("customer_credit")}
              >
                Customer credit
              </Button>
            </div>
            {outcome === "refund_now" ? (
              ...
              <p className="rounded-md bg-muted p-3 text-sm text-muted-foreground">
                No money leaves now. The amount remains available as customer
                credit.
              </p>
            )}
        ...
        <div className="sticky bottom-0 flex gap-2 border-t bg-background pt-3">
          <Button
