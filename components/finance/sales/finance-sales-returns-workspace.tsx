...
export function FinanceSalesReturnsWorkspace() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      toast.error(
        error.response?.data?.detail || "Could not load sales returns.",
      );
    } finally {
      setLoading(false);
    }
    ...
    <AppPage width="wide" density="compact">
      ...
      <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 sm:gap-3">
        ...
        <MetricCard
          ...
          value={formatMoney(
            documents
              .filter((item) => item.settlement_status === "customer_credit")
        ...
        <div className="flex items-center justify-between border-b bg-muted/30 px-4 py-3">
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              Select a credit note to see its returned items, settlement, and
              source sale.
            </p>
          </div>
        ...
        {documents.length ? (
            ...
            <DataList className="rounded-none border-x-0 border-y-0 lg:hidden">
              {documents.map((document) => (
                <ListRow
                  ...
                  description={`${formatDate(document.business_date)} · ${originalSaleLabel(document, salesById.get(document.original_document_id || 0))}`}
                  meta={
                  ...
                  trailing={
                    <span className="max-w-24 truncate rounded-full bg-muted px-2 py-1 text-xs">
                      {salesSettlementLabel(document.settlement_status)}
                    </span>
                  }
                  interactive
                  role="button"
                  ...
                  {documents.map((document) => (
                    ...
                    >
                      ...
                      <td className="p-3">
                        {originalSaleLabel(
                          document,
                          salesById.get(document.original_document_id || 0),
                        )}
                      </td>
          ...
          <div className="p-12 text-center">
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Refunding a completed order through this workflow creates its
      ...
      <TransactionDetailSheet
        ...
        detail={
          selectedDocument
            ? salesReturnDetail(
                selectedDocument,
                salesById.get(selectedDocument.original_document_id || 0),
              )
            : null
        }
      />
