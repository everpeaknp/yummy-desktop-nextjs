...
const minor = (value: string | number | null | undefined) =>
...
export function ManualJournalsClient() {
  ...
  const [reverseEntry, setReverseEntry] =
    useState<FinanceReportingEntryRead | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      toast.error(
        error.response?.data?.detail || "Could not load journal vouchers.",
      );
    } finally {
      setLoading(false);
    }
  ...
  const completeLines = lines.filter(
    (line) =>
      line.reportingHeadId && (minor(line.debit) > 0 || minor(line.credit) > 0),
  );
  const isBalanced =
  ...
  const openCreate = () => {
    setReference(
      `JV-${today().replaceAll("-", "")}-${String(Date.now()).slice(-6)}`,
    );
  ...
  const updateLine = (
    key: string,
    field: keyof Omit<JournalLine, "key">,
    value: string,
  ) => {
    setLines((current) =>
      current.map((line) => {
  ...
  const save = async () => {
    if (!reference.trim() || !memo.trim() || !isBalanced) {
      toast.error(
        "Add a reference, memo, and at least two balanced debit and credit lines.",
      );
      return;
    }
    setSaving(true);
    try {
      ...
      toast.error(
        error.response?.data?.detail || "Could not post journal voucher.",
      );
    } finally {
      setSaving(false);
    }
  ...
  const reverse = async () => {
    ...
    setSaving(true);
    try {
      await financeReportingApi.reverseManualJournal(reverseEntry.id, {
      ...
      toast.error(
        error.response?.data?.detail || "Could not reverse journal voucher.",
      );
    } finally {
      setSaving(false);
    }
        ...
        <Card>
          <CardContent className="p-8 text-center">
            ...
            <p className="mt-2 text-sm text-muted-foreground">
              You do not have permission to view journal vouchers.
            </p>
          </CardContent>
        </Card>
      </AppPage>
          ...
          {loading ? (
                  ...
                  return (
                    <div key={entry.id}>
                      <ListRow
                        ...
                        description={`${formatDate(entry.business_date)} · ${entry.lines.find((line) => line.description)?.description || "Journal voucher"}`}
                        meta={
                      ...
                      return (
                        <TableRow key={entry.id}>
                          ...
                          <TableCell className="max-w-md font-medium">
                            {entry.lines.find((line) => line.description)
            ...
            <div className="flex min-h-64 flex-col items-center justify-center px-6 text-center">
              ...
              <p className="mt-1 text-sm text-muted-foreground">
                Normal operations post automatically. Use a voucher only for a
                genuine accounting adjustment.
              </p>
            </div>
          )}
        </CardContent>
      ...
      <Dialog open={createOpen} onOpenChange={setCreateOpen}>
        <DialogContent className="max-h-[92dvh] w-[calc(100vw-1.5rem)] max-w-5xl overflow-y-auto rounded-2xl p-4 sm:p-6">
          <DialogHeader>
            ...
            <DialogDescription>
              Debits and credits must balance exactly. Choose only postable
              account heads.
            </DialogDescription>
          </DialogHeader>
                ...
                <div className="grid gap-3">
                  <div className="grid gap-1.5">
                    ...
                    <Select
                      ...
                      onValueChange={(value) =>
                        updateLine(line.key, "reportingHeadId", value)
                      }
                    >
          ...
          <div className="mt-2 hidden overflow-x-auto rounded-lg border lg:block">
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead className="min-w-48">Line note</TableHead>
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div
              className={`rounded-lg border px-4 py-3 text-sm ${isBalanced ? "border-emerald-200 bg-emerald-50 text-emerald-800" : "border-amber-200 bg-amber-50 text-amber-900"}`}
            >
              <span className="mr-4">Debit {money(totals.debit / 100)}</span>
              <span>Credit {money(totals.credit / 100)}</span>
              <strong className="ml-4">
                {isBalanced
                  ? "Balanced"
                  : `Difference ${money(Math.abs(totals.debit - totals.credit) / 100)}`}
              </strong>
            </div>
          </div>
          <DialogFooter>
      ...
      >
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              This creates an equal and opposite audit entry. The original
