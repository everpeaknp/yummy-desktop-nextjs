...
import { BedDouble, CalendarDays, ChevronRight, Loader2, Plus, Search, UserRound } from "lucide-react";
...
export function BookingsPanel({ restaurantId, canManage, refreshKey, onOpenBooking, onChanged }: Props) {
  ...
  }, [restaurantId, submittedSearch, status]);
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-col justify-between gap-4 md:flex-row md:items-end">
        <p className="text-sm text-muted-foreground">Search by guest, phone number, or booking code.</p>
        {canManage ? <Button className="h-11 rounded-xl" onClick={() => setFormOpen(true)}><Plus className="mr-2 h-4 w-4" />New booking</Button> : null}
      </div>

      <Card className="shadow-none"><CardContent className="flex flex-col gap-2 p-2.5 md:flex-row">
        <div className="relative flex-1"><Search className="absolute left-3 top-3.5 h-4 w-4 text-muted-foreground" /><Input className="h-11 rounded-xl pl-9" value={search} onChange={(event) => setSearch(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") { const next = search.trim(); if (next === submittedSearch) void load(); else setSubmittedSearch(next); } }} placeholder="Guest, phone, or confirmation code" /></div>
      ...
      <Card className="overflow-hidden shadow-sm"><CardContent className="p-0">
        {loading && !bookings.length ? <div className="flex min-h-64 items-center justify-center"><Loader2 className="h-7 w-7 animate-spin" /></div> : bookings.length ? <>
          <div className="divide-y md:hidden">{bookings.map((booking) => <button key={booking.id} type="button" onClick={() => onOpenBooking(booking.id)} className="flex w-full items-start gap-3 p-3.5 text-left transition-colors hover:bg-muted/40">
            <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-orange-500/10 text-orange-600"><UserRound className="h-4 w-4" /></span>
            <span className="min-w-0 flex-1"><span className="flex flex-wrap items-center gap-2"><span className="truncate font-semibold">{booking.primary_guest_name}</span><HotelStatusBadge value={booking.status} /></span><span className="mt-1.5 flex items-center gap-1.5 text-xs text-muted-foreground"><CalendarDays className="h-3.5 w-3.5" />{booking.arrival_date} - {booking.departure_date}</span><span className="mt-1 flex items-center gap-1.5 text-xs text-muted-foreground"><BedDouble className="h-3.5 w-3.5" />{bookingRooms(booking)}</span><span className="mt-1.5 block text-sm font-bold tabular-nums">{bookingTotal(booking)}</span></span>
