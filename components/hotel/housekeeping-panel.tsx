  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-center">
        ...
        <Input aria-label="Housekeeping date" className="h-11 w-full rounded-xl sm:w-44" type="date" value={businessDate} onChange={(event) => setBusinessDate(event.target.value)} />
      </div>
        ...
        {Object.entries(counts).map(([label, value]) => (
          <Card key={label} className="shadow-none">
            <CardContent className="p-3.5 sm:p-4">
              <p className="text-xs font-medium text-muted-foreground">{label.replace(/([A-Z])/g, " $1")}</p>
      ...
      <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-center">
        ...
        <div className="flex self-start rounded-xl border bg-muted/40 p-1 sm:self-auto">
          <Button size="sm" className="rounded-lg" variant={view === "current" ? "secondary" : "ghost"} onClick={() => setView("current")}>Current</Button>
          <Button size="sm" className="rounded-lg" variant={view === "history" ? "secondary" : "ghost"} onClick={() => setView("history")}>History</Button>
        </div>
      </div>
