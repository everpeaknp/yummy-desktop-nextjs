  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
        ...
        <div className="flex gap-2"><Input aria-label="Hotel business date" className="h-11 w-full rounded-xl sm:w-44" type="date" value={businessDate} onChange={(event) => setBusinessDate(event.target.value)} /><Button aria-label="Refresh day summary" className="h-11 w-11 rounded-xl" variant="outline" size="icon" onClick={() => void preview()} disabled={loading}><RefreshCw className={loading ? "h-4 w-4 animate-spin" : "h-4 w-4"} /></Button></div>
      </div>
