...
export function RatesPanel({ restaurantId, canManageRates, canManageSettings, refreshKey, onChanged }: Props) {
  ...
  return (
    <div className="space-y-4">
      <p className="text-sm text-muted-foreground">Set arrival times, booking options, and prices for specific dates.</p>
      <Tabs value={activeSection} onValueChange={setActiveSection}>
        <TabsList className="grid h-11 w-full grid-cols-3 rounded-xl border bg-muted/40 p-1">
          <TabsTrigger value="pricing" className="rounded-lg text-xs sm:text-sm">Daily prices</TabsTrigger>
          <TabsTrigger value="plans" className="rounded-lg text-xs sm:text-sm">Booking options</TabsTrigger>
          <TabsTrigger value="settings" className="rounded-lg text-xs sm:text-sm">Settings</TabsTrigger>
        </TabsList>
        <TabsContent value="settings" className="mt-4">
        {settings ? <Card className="shadow-none"><CardHeader className="p-4 pb-2"><CardTitle className="text-base">Hotel settings</CardTitle><CardDescription>These defaults are used when staff create a booking.</CardDescription></CardHeader><CardContent className="grid gap-3 p-4 pt-3 sm:grid-cols-2"><div className="space-y-2"><Label>Usual check-in time</Label><Input className="h-11" type="time" value={settings.default_checkin_time.slice(0, 5)} disabled={!canManageSettings} onChange={(event) => setSettings({ ...settings, default_checkin_time: event.target.value })} /></div><div className="space-y-2"><Label>Usual checkout time</Label><Input className="h-11" type="time" value={settings.default_checkout_time.slice(0, 5)} disabled={!canManageSettings} onChange={(event) => setSettings({ ...settings, default_checkout_time: event.target.value })} /></div><div className="space-y-2"><Label>Currency</Label><Input className="h-11" maxLength={3} value={settings.currency} disabled={!canManageSettings} onChange={(event) => setSettings({ ...settings, currency: event.target.value.toUpperCase() })} /></div><div className="space-y-2"><Label>Booking rules</Label><div className="space-y-2 rounded-xl border p-3 text-sm"><label className="flex items-center gap-2"><Checkbox checked={settings.require_clean_room_for_checkin} disabled={!canManageSettings} onCheckedChange={(checked) => setSettings({ ...settings, require_clean_room_for_checkin: checked === true })} />Only check guests into ready rooms</label><label className="flex items-center gap-2"><Checkbox checked={settings.allow_overbooking} disabled={!canManageSettings} onCheckedChange={(checked) => setSettings({ ...settings, allow_overbooking: checked === true })} />Allow more bookings than available rooms</label></div></div>{canManageSettings ? <Button className="h-11 sm:col-span-2" disabled={saving} onClick={() => void saveSettings()}><Save className="mr-2 h-4 w-4" />Save settings</Button> : null}</CardContent></Card> : null}
        </TabsContent>
        <TabsContent value="plans" className="mt-4">
        <Card className="shadow-none"><CardHeader className="p-4 pb-2"><CardTitle className="text-base">Booking options</CardTitle><CardDescription>Create choices such as flexible, breakfast included, or non-refundable.</CardDescription></CardHeader><CardContent className="space-y-3 p-4 pt-3">
      ...
      <Card className="shadow-none"><CardHeader className="p-4 pb-2"><CardTitle className="text-base">Price for a specific date</CardTitle><CardDescription>Use this when a room should cost more or less on a particular date.</CardDescription></CardHeader><CardContent className="grid gap-3 p-4 pt-3 sm:grid-cols-2">{roomTypes.length && ratePlans.length ? <><div className="space-y-2"><Label>Room type</Label><Select value={roomTypeId} onValueChange={(value) => { setRoomTypeId(value); const type = roomTypes.find((item) => item.id === Number(value)); if (type) setPrice(String(type.base_rate)); }}><SelectTrigger className="h-11"><SelectValue /></SelectTrigger><SelectContent>{roomTypes.map((type) => <SelectItem key={type.id} value={String(type.id)}>{type.name} · {hotelCurrency(type.base_rate)}</SelectItem>)}</SelectContent></Select></div><div className="space-y-2"><Label>Booking option</Label><Select value={ratePlanId} onValueChange={setRatePlanId}><SelectTrigger className="h-11"><SelectValue /></SelectTrigger><SelectContent>{ratePlans.map((plan) => <SelectItem key={plan.id} value={String(plan.id)}>{plan.name}</SelectItem>)}</SelectContent></Select></div><div className="space-y-2"><Label>Date</Label><Input className="h-11" type="date" value={stayDate} onChange={(event) => setStayDate(event.target.value)} /></div><div className="space-y-2"><Label>Nightly price</Label><Input className="h-11" type="number" min={0} step="0.01" value={price} onChange={(event) => setPrice(event.target.value)} /></div><div className="space-y-2"><Label>Minimum nights</Label><Input className="h-11" type="number" min={1} value={minStay} onChange={(event) => setMinStay(event.target.value)} /></div><div className="space-y-2"><Label>Arrival rules</Label><div className="space-y-2 rounded-xl border p-3 text-sm"><label className="flex items-center gap-2"><Checkbox checked={closedToArrival} onCheckedChange={(checked) => setClosedToArrival(checked === true)} />No check-in</label><label className="flex items-center gap-2"><Checkbox checked={closedToDeparture} onCheckedChange={(checked) => setClosedToDeparture(checked ===true)} />No checkout</label></div></div>{canManageRates ? <Button className="h-11 sm:col-span-2" disabled={saving || !roomTypeId || !ratePlanId || !price} onClick={() => void saveDailyRate()}><Save className="mr-2 h-4 w-4" />Save price</Button> : null}</> : <HotelEmptyState className="sm:col-span-2" title="Finish room setup first" description="Add a room type and booking option before setting prices." />}</CardContent></Card>
        </TabsContent>
      </Tabs>
    </div>
  );
