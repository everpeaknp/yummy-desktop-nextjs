...
export function PurchasesWorkspace({
    ...
    const handleReturn = async () => {
        ...
        if (!returningPurchase?.id || !returnReason.trim() || parsedReturnAmount <= 0) {
            toast.error("Enter the refund amount and a reason for this return.");
            return;
        }
        setReturning(true);
        try {
            const res = await apiClient.post(GeneralPurchaseApis.return(returningPurchase.id), {
        ...
        <div className="p-6 space-y-6 max-w-[1400px] mx-auto">
            ...
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div className="space-y-1">
                    ...
                    <h1 className="text-3xl font-bold tracking-tight">{returnedOnly ? "Purchase returns" : "Purchases"}</h1>
                ...
                <div className="flex items-center gap-2">
                    ...
                    {!returnedOnly ? <Button onClick={startPurchase}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <Card>
                    <CardContent className="p-4 flex items-center justify-between">
                        <div>
                            <p className="text-xs font-medium text-muted-foreground uppercase">Unpaid Purchases</p>
                                ...
                                <TableRow key={purchase.id} className={cn(purchase.status === 'cancelled' && "opacity-60")}>
                                    ...
                                    <TableCell className="text-right">
                                        ...
                                        {purchase.status === 'received' ? (
                                            <Button variant="ghost" size="sm" onClick={() => openReturn(purchase)} className="text-orange-700">
                                        ...
                                        <DropdownMenu>
                                            ...
                                            <DropdownMenuContent align="end" className="w-48">
                                                ...
                                                {purchase.status === 'received' && (
                                                    <DropdownMenuItem onClick={() => openReturn(purchase)}>
            ...
            />
            <Dialog open={Boolean(returningPurchase)} onOpenChange={(open) => { if (!open && !returning) setReturningPurchase(null); }}>
                <DialogContent className="sm:max-w-lg">
                    <DialogHeader>
                        <DialogTitle>Return purchase</DialogTitle>
                        <DialogDescription>
                            Return {returningPurchase?.purchase_name || "this purchase"}. This reverses the entire received purchase; use an inventory adjustment for a partial quantity correction.
                        </DialogDescription>
                    </DialogHeader>
                    <div className="grid gap-2 py-2">
                        <Label htmlFor="purchase-return-amount">Refund amount</Label>
                        <Input id="purchase-return-amount" type="number" min="0.01" step="0.01" max={Number(returningPurchase?.total_cost || 0)} value={returnAmount} onChange={(event) => setReturnAmount(event.target.value)} />
                        <Label htmlFor="purchase-return-reason">Reason</Label>
                        <Textarea id="purchase-return-reason" value={returnReason} onChange={(event) => setReturnReason(event.target.value)} placeholder="Damaged, incorrect item, supplier return..." />
                    </div>
                    <DialogFooter>
                        <Button variant="outline" onClick={() => setReturningPurchase(null)} disabled={returning}>Cancel</Button>
                        <Button onClick={() => void handleReturn()} disabled={returning || !returnReason.trim() || Number(returnAmount || 0) <= 0}>
