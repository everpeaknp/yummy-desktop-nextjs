...
export function SupplierDetailWorkspace({
  ...
  const [tab, setTab] = useState<
    "activity" | "bills" | "returns" | "payments" | "open" | "statement"
  >("activity");
  ...
  const [receiptAccount, setReceiptAccount] =
  ...
  const totalPurchases = purchases.reduce(
    (sum, purchase) => sum + money(purchase.total_cost),
    0,
  );
  const totalReturns = returns.reduce(
    (sum, purchaseReturn) => sum + money(purchaseReturn.total_cost),
    0,
  );
  const paymentsOut = payments
    .filter((entry: any) => entry.entry_type === "supplier_payment")
    .reduce((sum, entry) => sum + money(entry.amount), 0);
  const moneyReceived = payments
    .filter((entry: any) =>
      ["supplier_refund_received", "supplier_payment_received"].includes(
        entry.entry_type,
      ),
    )
    .reduce((sum, entry) => sum + money(entry.amount), 0);
  ...
  const fifoAllocated = fifoPreview.reduce(
    (sum: number, bill: any) => sum + money(bill.allocationAmount),
    0,
  );
  ...
  const submitPayment = async () => {
    ...
    if (
      ...
      openBills.some(
        (bill: any) =>
          money(allocations[bill.id]) > money(bill.open_amount) + 0.004,
      )
    ) {
      toast.error(
        "A bill allocation cannot exceed that bill's remaining balance.",
      );
      return;
    }
    setPaying(true);
    try {
      await apiClient.post(SupplierApis.pay(supplierId, user.restaurant_id), {
        ...
        allocations: manualAllocation
          ? openBills
              ...
              .map((bill: any) => ({
                target_entry_id: bill.id,
                amount: money(allocations[bill.id]),
              }))
          : [],
      });
      toast.success(
        manualAllocation
          ? "Supplier payment allocated to the selected bills."
          : "Supplier payment applied oldest bill first.",
      );
      setPayOpen(false);
      await load();
    } catch (error: any) {
      toast.error(
        error.response?.data?.detail ||
          "Could not record the supplier payment.",
      );
    } finally {
      setPaying(false);
    }
  ...
  const submitReceipt = async () => {
    if (!user?.restaurant_id || !receiptAccount || money(receiptAmount) <= 0) {
      toast.error(
        "Enter the amount received and choose the bank or cash drawer.",
      );
      return;
    }
    setReceiving(true);
    try {
      await apiClient.post(
        SupplierApis.receive(supplierId, user.restaurant_id),
        {
          amount: money(receiptAmount),
          account_type: receiptAccount.account_type,
          account_id: receiptAccount.id,
          payment_method:
            receiptAccount.account_type === "drawer" ? "cash" : "bank_transfer",
      ...
      toast.error(
        error.response?.data?.detail ||
          "Could not record the supplier payment received.",
      );
    } finally {
      setReceiving(false);
    }
  ...
  if (loading)
    return (
      <div className="flex min-h-[360px] items-center justify-center text-muted-foreground">
        <Loader2 className="mr-2 h-5 w-5 animate-spin" /> Loading supplier
        workspace…
      </div>
    );
    ...
    <div className="mx-auto max-w-[1600px] space-y-4 px-4 pb-24 pt-4 lg:px-8 lg:pb-8">
            ...
            <div>
              ...
              <p className="mt-1 text-sm text-muted-foreground">
                {[supplier.contact_name, supplier.phone, supplier.email]
                  .filter(Boolean)
                  .join(" · ") || "Supplier details and settlement history"}
              </p>
            </div>
          </div>
          <div>
            ...
            >
              {payable > 0
                ? `${formatCurrency(payable)} payable`
                : supplierCredit > 0
                  ? `${formatCurrency(supplierCredit)} supplier credit`
                  : "Settled"}
            </p>
          </div>
        </div>
        ...
        <div className="space-y-5">
          <section>
            ...
            <DetailGrid className="mt-3 gap-y-3">
              <DetailField
                label="Contact"
                value={supplier.contact_name || "Not recorded"}
              />
              <DetailField
                label="Address"
                value={supplier.address || "Not recorded"}
              />
              <div>
        ...
        <div className="space-y-5">
          <section>
            ...
            <div className="mt-3 overflow-hidden rounded-xl border bg-card sm:grid sm:grid-cols-2 sm:divide-x">
              ...
              <Metric
                label="Payments out"
                value={formatCurrency(paymentsOut)}
              />
              <Metric
                label="Money received"
                value={formatCurrency(moneyReceived)}
              />
            </div>
          </section>
          <section className="space-y-3">
            ...
            <div className="overflow-hidden rounded-xl border bg-card divide-y">
              <Button
                ...
                onClick={() => setTab("open")}
              >
                <span>Open purchase bills</span>
                <span className="flex items-center gap-2 text-muted-foreground">
                  <span className="tabular-nums">{openBills.length}</span>
          ...
          <section className="space-y-2">
            <div className="flex items-center justify-between">
              ...
              <Button
                ...
                className="h-11 w-11"
                onClick={load}
                aria-label="Refresh supplier activity"
              >
            ...
            {tab === "bills" && (
              <SupplierBillRegister
                purchases={purchases}
                openBills={openBills}
                onOpen={(purchase) =>
                  setSelectedDetail(
                    purchaseDocumentDetail(purchase, statement, returns),
                  )
                }
              />
            )}
            {tab === "returns" && (
              <SupplierReturnRegister
                returns={returns}
                onOpen={(purchaseReturn) =>
                  setSelectedDetail(purchaseReturnDetail(purchaseReturn))
                }
              />
            )}
            {tab === "payments" && (
            ...
            {tab === "open" && (
              <OpenBillList
                ...
                onReturn={(purchaseId) =>
                  router.push(
                    `/inventory/purchases/returns?purchase_id=${purchaseId}`,
                  )
                }
              />
            )}
            {tab === "statement" && (
              <StatementWithAllocations
                ...
                onOpenAllocation={(allocation) =>
                  setSelectedDetail(
                    settlementAllocationDetail(allocation, "supplier"),
                  )
                }
              />
            )}
      ...
      <Dialog open={payOpen} onOpenChange={setPayOpen}>
        <DialogContent className="max-h-[92vh] overflow-y-auto sm:max-w-[680px]">
          <DialogHeader>
            ...
            <DialogDescription>
              Money leaves the selected account once. Bills are applied oldest
              due first unless you choose exact allocations.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-5 py-2">
            ...
            <div className="rounded-lg border p-3">
              <label className="flex cursor-pointer items-center gap-2 text-sm font-medium">
                ...
                />{" "}
                Allocate to specific purchase bills
              </label>
              <p className="mt-1 text-xs text-muted-foreground">
                Leave this off for FIFO. A smaller allocation leaves the rest as
            ...
            {manualAllocation && (
              <div className="space-y-2 rounded-lg border p-3">
                <div className="flex items-center justify-between">
                  <p className="font-medium">Open purchase bills</p>
                ...
                {openBills.length === 0 ? (
                  <p className="text-sm text-muted-foreground">
                    There are no open purchase bills to allocate.
                  </p>
                ) : (
                    ...
                    >
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground">
                          Open {formatCurrency(bill.open_amount)} ·{" "}
                          {bill.source_reference || "No reference"}
                        </p>
                      </div>
                      <Input
                        type="number"
                        min="0"
                        max={String(bill.open_amount)}
                        step="0.01"
      ...
      <Dialog open={receiveOpen} onOpenChange={setReceiveOpen}>
        <DialogContent className="sm:max-w-[560px]">
          <DialogHeader>
            <DialogTitle>Receive payment from {supplier.name}</DialogTitle>
            <DialogDescription>
              This is not income. The money is posted to the selected account
              and retained as supplier credit for future purchase bills.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-5 py-2">
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                placeholder="Supplier receipt, bank transfer or cheque #"
              />
            </div>
            <div className="space-y-2">
              <Label>Notes</Label>
              <Textarea
                ...
                placeholder="Why the supplier paid this amount"
              />
            </div>
            <div className="rounded-lg border bg-muted/30 p-3 text-sm">
              <p className="font-medium">Booked as supplier credit</p>
              <p className="mt-1 text-muted-foreground">
                This credit reduces what you owe this supplier on a future bill.
                It does not increase sales or other income.
              </p>
            </div>
          </div>
          <DialogFooter>
            ...
            <Button onClick={submitReceipt} disabled={receiving}>
              {receiving ? (
                <Loader2 className="mr-2 h-4 w-4 animate-spin" />
              ) : null}
              Record payment in
      ...
      return {
        ...
        tone:
          entry.entry_side === "debt"
            ...
            : ("positive" as const),
        amountLabel:
          entry.entry_side === "debt" ? "Amount due" : "Supplier credit",
        amountClass:
...
function supplierActivityOpenLabel(entry: any) {
  ...
  return `Open ${formatCurrency(openAmount)}`;
}
            ...
            return (
              ...
              >
                ...
                <div className="text-right">
                  <p
                    className={`text-sm font-semibold ${presentation.amountClass}`}
                  >
                    {presentation.amountLabel} {formatCurrency(entry.amount)}
                  </p>
                  <p className="text-xs text-muted-foreground">
                    {supplierActivityOpenLabel(entry)}
                  </p>
                </div>
              </button>
            );
        ...
        entries.map((entry) => {
          ...
          return (
            <PartyActivityRow
              ...
              metadata={`${formatDate(entry.financial_date)} - ${entry.source_reference || entry.entry_type.replaceAll("_", " ")}`}
              amount={formatCurrency(entry.amount)}
              amountTone={presentation.tone}
              status={supplierActivityOpenLabel(entry)}
              onClick={() => onOpen(entry)}
            />
          );
        })
...
function SupplierBillRegister({
  ...
  purchases: any[];
  openBills: any[];
  onOpen: (purchase: any) => void;
}) {
  const openBySource = new Map(openBills.map((bill) => [bill.source_id, bill]));

  return (
    <section className="space-y-2">
      <h3 className="text-sm font-semibold">Purchase bills</h3>
      <div className="overflow-hidden rounded-xl border bg-card divide-y">
        {purchases.length ? (
          purchases.map((purchase) => {
            const bill = openBySource.get(purchase.id);
            return (
              <PartyActivityRow
                key={purchase.id}
                kind="purchase"
                title={purchase.reference_number || `Purchase #${purchase.id}`}
                metadata={`${formatDate(purchase.purchase_date)} - ${purchase.payment_status || "unpaid"}`}
                amount={formatCurrency(purchase.total_cost)}
                amountTone={bill ? "warning" : "default"}
                status={
                  bill ? `${formatCurrency(bill.open_amount)} open` : "Settled"
                }
          ...
          <p className="px-4 py-6 text-sm text-muted-foreground">
            No purchases have been recorded for this supplier.
          </p>
        )}
      </div>
    </section>
...
function SupplierReturnRegister({
  returns,
  onOpen,
}: {
  returns: any[];
  onOpen: (purchaseReturn: any) => void;
}) {
  return (
    <section className="space-y-2">
      <h3 className="text-sm font-semibold">Purchase returns</h3>
      <div className="overflow-hidden rounded-xl border bg-card divide-y">
        {returns.length ? (
          returns.map((entry) => (
            <PartyActivityRow
              key={entry.id}
              kind="return"
              title={entry.return_number || `Purchase return #${entry.id}`}
              metadata={`${formatDate(entry.return_date)} - ${String(entry.settlement_type || "return").replaceAll("_", " ")}`}
          ...
          <p className="px-4 py-6 text-sm text-muted-foreground">
            No purchase returns have been recorded for this supplier.
          </p>
        )}
      </div>
    </section>
...
function SupplierPaymentRegister({
  entries,
  onOpen,
}: {
  entries: any[];
        ...
        {entries.length ? (
          entries.map((entry) => (
            <PartyActivityRow
              ...
              kind="payment"
              title={entry.display_name}
              metadata={`${entry.payment_method?.replaceAll("_", " ") || "Payment"} - ${formatDate(entry.financial_date)}`}
              amount={formatCurrency(entry.amount)}
              amountTone="positive"
              status={supplierActivityOpenLabel(entry)}
              onClick={() => onOpen(entry)}
    ...
    <Card>
      ...
      <CardContent className="space-y-2">
        {purchases.length ? (
              ...
              >
                <div>
                  <p className="font-medium">
                    {purchase.reference_number || `Purchase #${purchase.id}`}
                  </p>
                  <p className="text-xs text-muted-foreground">
                ...
                <div className="text-right">
                  ...
                  <p className="text-xs text-muted-foreground">
                    {bill
                      ? `${formatCurrency(bill.open_amount)} open`
                      : "Settled"}
                  </p>
                </div>
              </button>
          ...
          <Empty text="No purchases have been recorded for this supplier." />
        )}
      </CardContent>
    </Card>
  );
        ...
        {returns.length ? (
          returns.map((entry) => (
            <button
              ...
              key={entry.id}
              onClick={() => onOpen(entry)}
              className="flex w-full items-center justify-between rounded-lg border p-3 text-left transition-colors hover:bg-muted/40 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
            >
              <div>
                <p className="font-medium">
                  {entry.return_number || `Purchase return #${entry.id}`}
                </p>
                <p className="text-xs text-muted-foreground">
                  {formatDate(entry.return_date)} ·{" "}
                  {String(entry.settlement_type || "").replaceAll("_", " ")}
                </p>
              </div>
          ...
          <Empty text="No purchase returns have been recorded for this supplier." />
        )}
      </CardContent>
    </Card>
  );
          ...
          entries.map((entry) => (
            ...
            >
              ...
              <Badge variant="outline">
                {entry.entry_side === "debt" ? "Bill" : "Payment / credit"}
              </Badge>
              <div className="text-right text-sm">
                <p>{formatCurrency(entry.amount)}</p>
                <p className="text-muted-foreground">
                  Open {formatCurrency(entry.open_amount)}
                </p>
              </div>
            </button>
          ))
    ...
    <div className="rounded-lg border bg-muted/20 p-3">
      ...
      <div className="mt-3 space-y-2">
        {lines.length ? (
          ...
          <p className="text-sm text-muted-foreground">
            No open bills will be settled. This becomes a supplier advance.
          </p>
        )}
      </div>
      <div className="mt-3 flex items-center justify-between border-t pt-3 text-sm">
        <span>Unallocated supplier credit</span>
        <span className="font-semibold">{formatCurrency(credit)}</span>
      </div>
    </div>
  );
}
    ...
    <Card>
      ...
      <CardContent className="space-y-2">
        {entries.length ? (
              ...
              <div className="text-right">
                <Badge variant="outline">
                  {entry.entry_type === "supplier_payment"
                    ? "Payment out"
                    : entry.entry_type === "supplier_payment_received"
                      ? "Payment in · credit"
                      : "Refund received"}
                </Badge>
          ...
          <Empty text="No supplier payments have been recorded." />
        )}
      </CardContent>
    </Card>
  );
...
function OpenBillList({
  ...
  bills: any[];
  onReturn: (purchaseId: number) => void;
  onOpen: (entry: any) => void;
}) {
  return (
    <Card>
        ...
        {bills.length ? (
          bills.map((bill) => (
            <div
              ...
              onClick={() => onOpen(bill)}
              onKeyDown={(event) => {
                if (event.key === "Enter" || event.key === " ") onOpen(bill);
              }}
              className="flex cursor-pointer flex-col gap-3 rounded-lg border p-3 text-left transition-colors hover:bg-muted/40 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring sm:flex-row sm:items-center sm:justify-between"
            >
              <div>
                <p className="font-medium">
                  {bill.source_reference || bill.display_name}
                </p>
                <p className="text-xs text-muted-foreground">
                  {formatDate(bill.financial_date)} · Original{" "}
                  {formatCurrency(bill.amount)} · Remaining{" "}
                  {formatCurrency(bill.open_amount)}
                </p>
              </div>
              {bill.source_type === "inventory_purchase" && bill.source_id ? (
                <Button
                  variant="outline"
                  size="sm"
                  onClick={(event) => {
                    event.stopPropagation();
                    onReturn(bill.source_id);
                  }}
                >
                  Return items
                </Button>
          ...
          <Empty text="There are no unpaid purchase bills." />
        )}
      </CardContent>
    </Card>
  );
...
function StatementWithAllocations({
  ...
  return (
    <div className="space-y-4">
      <StatementList entries={entries} onOpen={onOpen} />
      <Card>
        <CardHeader className="pb-3">
