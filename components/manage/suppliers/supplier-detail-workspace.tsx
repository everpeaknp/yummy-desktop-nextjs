    ...
    <div className="mx-auto max-w-[1440px] space-y-6 p-4 md:p-6">
      ...
      <div className="grid gap-4 lg:grid-cols-[300px_1fr]">
        ...
        <div className="space-y-4">
          ...
          {tab === "bills" && <BillList purchases={purchases} openBills={openBills} />}
          {tab === "returns" && <ReturnList returns={returns} onOpen={(returnId) => router.push(`/inventory/purchases/returns?return_id=${returnId}`)} />}
          {tab === "payments" && <PaymentList entries={payments} />}
          {tab === "open" && <OpenBillList bills={openBills} onReturn={(purchaseId) => router.push(`/inventory/purchases/returns?purchase_id=${purchaseId}`)} />}
          {tab === "statement" && <StatementWithAllocations entries={activity} allocations={statement?.allocations || []} />}
        </div>
      </div>
...
function ActivityList({ entries }: { entries: any[] }) {
  return <Card>
    ...
    <CardContent className="space-y-1">
      {entries.length ? entries.map((entry) => {
        ...
        return <div key={entry.id} className="flex items-center justify-between gap-4 rounded-md px-2 py-3 hover:bg-muted/50">
          <div>
...
function BillList({ purchases, openBills }: { purchases: any[]; openBills: any[] }) { const openBySource = new Map(openBills.map((bill) => [bill.source_id, bill])); return <Card><CardHeader className="pb-3"><CardTitle className="text-base">Purchase bills</CardTitle></CardHeader><CardContent className="space-y-2">{purchases.length ? purchases.map((purchase) => { const bill = openBySource.get(purchase.id); return <div key={purchase.id} className="flex items-center justify-between gap-4 rounded-lg border p-3"><div><p className="font-medium">{purchase.reference_number || `Purchase #${purchase.id}`}</p><p className="text-xs text-muted-foreground">{formatDate(purchase.purchase_date)} · {purchase.payment_status || "unpaid"}</p></div><div className="text-right"><p className="font-medium">{formatCurrency(purchase.total_cost)}</p><p className="text-xs text-muted-foreground">{bill ? `${formatCurrency(bill.open_amount)} open` : "Settled"}</p></div></div>; }) : <Empty text="No purchases have been recorded for this supplier." />}</CardContent></Card>; }
function ReturnList({ returns, onOpen }: { returns: any[]; onOpen: (returnId: number) => void }) { return <Card><CardHeader className="pb-3"><CardTitle className="text-base">Purchase returns</CardTitle></CardHeader><CardContent className="space-y-2">{returns.length ? returns.map((entry) => <button type="button" key={entry.id} onClick={() => onOpen(entry.id)} className="flex w-full items-center justify-between rounded-lg border p-3 text-left transition-colors hover:bg-muted/40 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"><div><p className="font-medium">Purchase return #{entry.id}</p><p className="text-xs text-muted-foreground">{formatDate(entry.return_date)} · {String(entry.settlement_type || "").replaceAll("_", " ")}</p></div><p className="font-medium text-emerald-600">{formatCurrency(entry.total_cost)}</p></button>) : <Empty text="No purchase returns have been recorded for this supplier." />}</CardContent></Card>; }
function StatementList({ entries }: { entries: any[] }) { return <Card><CardHeader className="pb-3"><CardTitle className="text-base">Open-item statement</CardTitle></CardHeader><CardContent className="space-y-2">{entries.length ? entries.map((entry) => <div key={entry.id} className="grid grid-cols-[1fr_auto_auto] items-center gap-4 rounded-lg border p-3"><div><p className="font-medium">{entry.display_name}</p><p className="text-xs text-muted-foreground">{formatDate(entry.financial_date)} · {entry.status}</p></div><Badge variant="outline">{entry.entry_side === "debt" ? "Bill" : "Payment / credit"}</Badge><div className="text-right text-sm"><p>{formatCurrency(entry.amount)}</p><p className="text-muted-foreground">Open {formatCurrency(entry.open_amount)}</p></div></div>) : <Empty text="No statement entries have been recorded." />}</CardContent></Card>; }
...
function PaymentList({ entries }: { entries: any[] }) { return <Card><CardHeader className="pb-3"><CardTitle className="text-base">Payments in and out</CardTitle></CardHeader><CardContent className="space-y-2">{entries.length ? entries.map((entry) => <div key={entry.id} className="flex items-center justify-between gap-4 rounded-lg border p-3"><div><p className="font-medium">{entry.display_name}</p><p className="text-xs text-muted-foreground">{formatDate(entry.financial_date)} · {entry.payment_method?.replaceAll("_", " ") || "No payment method recorded"}</p></div><div className="text-right"><Badge variant="outline">{entry.entry_type === "supplier_payment" ? "Payment out" : entry.entry_type === "supplier_payment_received" ? "Payment in · credit" : "Refund received"}</Badge><p className="mt-1 font-medium">{formatCurrency(entry.amount)}</p></div></div>) : <Empty text="No supplier payments have been recorded." />}</CardContent></Card>; }

function OpenBillList({ bills, onReturn }: { bills: any[]; onReturn: (purchaseId: number) => void }) { return <Card><CardHeader className="pb-3"><CardTitle className="text-base">Open bills ready for payment</CardTitle></CardHeader><CardContent className="space-y-2">{bills.length ? bills.map((bill) => <div key={bill.id} className="flex flex-col gap-3 rounded-lg border p-3 sm:flex-row sm:items-center sm:justify-between"><div><p className="font-medium">{bill.source_reference || bill.display_name}</p><p className="text-xs text-muted-foreground">{formatDate(bill.financial_date)} · Original {formatCurrency(bill.amount)} · Remaining {formatCurrency(bill.open_amount)}</p></div>{bill.source_type === "inventory_purchase" && bill.source_id ? <Button variant="outline" size="sm" onClick={() => onReturn(bill.source_id)}>Return items</Button> : null}</div>) : <Empty text="There are no unpaid purchase bills." />}</CardContent></Card>; }

function StatementWithAllocations({ entries, allocations }: { entries: any[]; allocations: any[] }) { return <div className="space-y-4"><StatementList entries={entries} /><Card><CardHeader className="pb-3"><CardTitle className="text-base">Settlement allocations</CardTitle></CardHeader><CardContent className="space-y-2">{allocations.length ? allocations.map((allocation) => <div key={allocation.id} className="grid gap-2 rounded-lg border p-3 sm:grid-cols-[1fr_auto_1fr_auto] sm:items-center"><div><p className="text-sm font-medium">{allocation.source_reference || allocation.source_display_name || "Supplier payment"}</p><p className="text-xs text-muted-foreground">Payment / credit</p></div><span className="text-center text-muted-foreground">applied to</span><div><p className="text-sm font-medium">{allocation.target_reference || allocation.target_display_name || "Purchase bill"}</p><p className="text-xs text-muted-foreground">{formatDate(allocation.financial_date)}</p></div><p className="text-right font-semibold">{formatCurrency(allocation.amount)}</p></div>) : <Empty text="No payment allocations have been recorded." />}</CardContent></Card></div>; }
