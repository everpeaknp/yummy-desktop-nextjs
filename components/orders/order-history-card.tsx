...
function settlementMeta(
  ...
  const methods = new Set(
    successful.map((payment) => String(payment.method || "").toLowerCase()),
  );
  if (methods.has("room_charge")) {
    return {
      label: "Charged to room",
      balanceDue: 0,
  ...
  if (settlement) {
    ...
    if (status === "partially_returned") {
      return {
        ...
        balanceDue: Number(settlement.balance_due || 0),
    ...
    return {
      label: "Unpaid",
      balanceDue: Number(settlement.balance_due || order.grand_total || 0),
      tone: "border-orange-200 bg-orange-50 text-orange-700",
    };
  }
...
export function OrderHistoryCard({ order, settlement }: OrderHistoryCardProps) {
  ...
  const shownItems = items
    .slice(0, 3)
    .map((item) => item.name_snapshot || item.item_name || "Item");
  ...
  const timeLabel =
    date && !Number.isNaN(date.getTime())
      ...
      : "—";
  const tableLabel =
    order.table_name || (order.channel === "table" ? "Table order" : "Order");
  const orderNumber = order.restaurant_order_id || order.id;
  const customer =
    order.customer_name ||
    (order.channel === "table" ? "Walk-in guest" : "Walk-in");
  const currency = (restaurant as any)?.currency || "NPR";
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="flex min-w-0 items-start gap-3">
          ...
          <div className="min-w-0">
            <h3 className="truncate text-sm font-bold leading-5 text-foreground sm:text-[17px] sm:leading-6">
              {tableLabel}
            </h3>
          ...
          {payment.balanceDue > 0 &&
            String(order.status).toLowerCase() === "completed" && (
              <p className="mb-0.5 text-xs font-semibold text-orange-700">
                {formatCurrency(payment.balanceDue, currency)} due
              </p>
            )}
