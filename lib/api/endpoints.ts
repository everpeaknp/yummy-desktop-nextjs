...
export const UserAccessScopeApis = {
  ...
  upsert: (userId: number | string, scopeKey: string) =>
    `/users/${userId}/access-scopes/${encodeURIComponent(scopeKey)}/`,
  remove: (userId: number | string, scopeKey: string) =>
    `/users/${userId}/access-scopes/${encodeURIComponent(scopeKey)}/`,
};
...
export const OrderApis = {
  ...
  payAllGuestBills: (orderId: number) =>
    `/orders/${orderId}/guest-bills/pay-all`,
  transferGuestBillItem: (orderId: number) =>
    `/orders/${orderId}/guest-bills/transfer-item`,
  mergeGuestBills: (orderId: number) => `/orders/${orderId}/guest-bills/merge`,
  cancelGuestBillSplit: (orderId: number) =>
    `/orders/${orderId}/guest-bills/cancel-split`,
  transferGuestBillTable: (orderId: number) =>
    `/orders/${orderId}/guest-bills/transfer-table`,
};

export const PaymentApis = {
  fonepayQr: "/payments/fonepay/qr",
  fonepayStatus: (prn: string) =>
    `/payments/fonepay/${encodeURIComponent(prn)}/status`,
};
...
export const MenuApis = {
  getMenusByRestaurant: (restaurantId: number, itemCategoryId?: number) => {
    const params = new URLSearchParams();
    if (itemCategoryId)
      params.append("item_category_id", itemCategoryId.toString());
    const query = params.toString();
    return query
      ? `/menus/restaurant/${restaurantId}?${query}`
      : `/menus/restaurant/${restaurantId}`;
  },
  getMenusGroupedByRestaurant: (restaurantId: number) =>
    `/menus/restaurant/${restaurantId}/grouped`,
  menuSummary: (
    restaurantId: number,
    { categoryId, fields }: { categoryId?: number; fields?: string } = {},
  ) => {
    const params = new URLSearchParams();
    ...
    const query = params.toString();
    return query
      ? `/menus/restaurant/${restaurantId}/summary?${query}`
      : `/menus/restaurant/${restaurantId}/summary`;
  },
...
export const TableApis = {
  ...
  tableSummary: (restaurantId: number, fields?: string, spaceKind?: string) => {
    ...
    const query = params.toString();
    return query
      ? `/restaurants/tables/summary/${restaurantId}?${query}`
      : `/restaurants/tables/summary/${restaurantId}`;
  },
...
export const TableTypeApis = {
  ...
  createTableType: (restaurantId: number) =>
    `/restaurants/table-types/${restaurantId}`,
  updateTableType: (tableTypeId: number) =>
    `/restaurants/table-types/${tableTypeId}`,
  deleteTableType: (tableTypeId: number) =>
    `/restaurants/table-types/${tableTypeId}`,
};
...
export const AdminManagementApis = {
  restaurantAdmins: (restaurantId: number) =>
...
export const CustomerApis = {
  listCustomers: (restaurantId: number) =>
    `/customers?restaurant_id=${restaurantId}`,
...
export const InventoryApis = {
  ...
  valuation: (restaurantId: number) =>
    `/inventory/valuation?restaurant_id=${restaurantId}`,
...
export const AnalyticsApis = {
  ...
  menuDetails: ({
    ...
    pageSize?: number;
    sortBy?:
      "revenue" | "quantity_sold" | "name" | "avg_price" | "category" | string;
...
export const ItemCategoryApis = {
  getItemCategories: (restaurantId: number) =>
    `/item-categories/restaurant/${restaurantId}`,
  createItemCategory: (restaurantId: number) =>
...
type AccountingInstrumentParams = Pick<
  AccountingCoreParams,
...
type AccountingVatExportParams = Pick<
  AccountingCoreParams,
  "restaurantId" | "dateFrom" | "dateTo"
> & {
  status?: string;
...
type AccountingDrilldownParams = Pick<
  AccountingCoreParams,
...
const buildAccountingDrilldownQuery = ({
  ...
  if (journalEntryId)
    params.append("journal_entry_id", journalEntryId.toString());
  if (journalLineId) params.append("journal_line_id", journalLineId.toString());
  if (financeEventId)
    params.append("finance_event_id", financeEventId.toString());
...
export const FinanceApis = {
  overview: (params: FinanceCoreParams) =>
    `/finance/overview?${buildFinanceQuery(params).toString()}`,
  ...
  expenses: (params: FinanceCoreParams) =>
    `/finance/expenses?${buildFinanceQuery(params).toString()}`,
  receivables: (params: FinanceCoreParams) =>
    `/finance/receivables?${buildFinanceQuery(params).toString()}`,
  reconciliation: (params: FinanceCoreParams) =>
    `/finance/reconciliation?${buildFinanceQuery(params).toString()}`,
};
...
export const AccountingApis = {
  inventoryProfiles: (restaurantId: number) =>
    `/accounting/inventory/profiles?restaurant_id=${restaurantId}`,
  createInventoryProfile: () => "/accounting/inventory/profiles",
  updateInventoryProfile: (profileId: number, restaurantId: number) =>
    `/accounting/inventory/profiles/${profileId}?restaurant_id=${restaurantId}`,
  assignInventoryProfile: (itemId: number, restaurantId: number) =>
    `/accounting/inventory/items/${itemId}/profile?restaurant_id=${restaurantId}`,
  inventoryAdoptions: (restaurantId: number) =>
    `/accounting/inventory/adoptions?restaurant_id=${restaurantId}`,
  createInventoryAdoption: () => "/accounting/inventory/adoptions",
  updateInventoryAdoptionLine: (
    adoptionId: number,
    itemId: number,
    restaurantId: number,
  ) =>
    `/accounting/inventory/adoptions/${adoptionId}/items/${itemId}?restaurant_id=${restaurantId}`,
  postInventoryAdoption: (adoptionId: number, restaurantId: number) =>
    `/accounting/inventory/adoptions/${adoptionId}/post?restaurant_id=${restaurantId}`,
  inventoryLegacyAudit: (restaurantId: number) =>
    `/accounting/inventory/legacy-audit?restaurant_id=${restaurantId}`,
  ...
  dayCloses: (
    params: AccountingCoreParams & { skip?: number; limit?: number },
  ) => {
    const query = buildAccountingQuery(params);
  ...
  updateOpeningBalance: (batchId: number) =>
    `/accounting/opening-balances/${batchId}`,
  postOpeningBalance: (batchId: number) =>
    `/accounting/opening-balances/${batchId}/post`,
  reverseOpeningBalance: (batchId: number) =>
    `/accounting/opening-balances/${batchId}/reverse`,
  ...
  updateJournalVoucher: (entryId: number) =>
    `/accounting/journal-vouchers/${entryId}`,
  submitJournalVoucher: (entryId: number) =>
    `/accounting/journal-vouchers/${entryId}/submit`,
  approveJournalVoucher: (entryId: number) =>
    `/accounting/journal-vouchers/${entryId}/approve`,
  rejectJournalVoucher: (entryId: number) =>
    `/accounting/journal-vouchers/${entryId}/reject`,
  postJournalVoucher: (entryId: number) =>
    `/accounting/journal-vouchers/${entryId}/post`,
  ...
  reverseJournalEntry: (entryId: number) =>
  ...
  supplierStatement: (
    params: Pick<
      AccountingLedgerParams,
      "restaurantId" | "dateFrom" | "dateTo" | "supplierId"
    >,
  ) =>
  ...
  confirmSettlementBank: (batchId: number) =>
    `/accounting/settlements/${batchId}/confirm-bank`,
  matchSettlement: (batchId: number) =>
    `/accounting/settlements/${batchId}/match`,
  approveSettlementVariance: (batchId: number) =>
    `/accounting/settlements/${batchId}/approve-variance`,
  postSettlement: (batchId: number) =>
    `/accounting/settlements/${batchId}/post`,
  ...
  periodPreflight: (periodId: number) =>
    `/accounting/periods/${periodId}/preflight`,
  softClosePeriod: (periodId: number) =>
    `/accounting/periods/${periodId}/soft-close`,
  lockPeriod: (periodId: number, force = false) =>
    `/accounting/periods/${periodId}/lock?force=${force}`,
  ...
  mappings: (
    params: Pick<AccountingCoreParams, "restaurantId" | "businessLine">,
  ...
  reverseRepostMappingException: () =>
    "/accounting/mapping-exceptions/reverse-repost",
...
export const KotApis = {
  ...
  getKotUpdatesByRestaurant: (restaurantId: number) =>
    `/kots/restaurants/${restaurantId}`,
  searchKots: "/kots/search",
  updateKotStatus: (kotId: number) => `/kots/${kotId}/status`,
  updateKotItemFulfillment: (kotId: number, itemId: number) =>
    `/kots/${kotId}/items/${itemId}/fulfillment`,
  markKotItemAll: (kotId: number, itemId: number) =>
    `/kots/${kotId}/items/${itemId}/fulfillment/mark-all`,
  markAllKotItems: (kotId: number) => `/kots/${kotId}/fulfillment/mark-all`,
  rejectKot: (kotId: number) => `/kots/${kotId}/reject`,
  rejectKotItem: (kotId: number, itemId: number) =>
    `/kots/${kotId}/items/${itemId}/reject`,
  acceptKotItem: (kotId: number, itemId: number) =>
    `/kots/${kotId}/items/${itemId}/accept`,
  getKotActivity: (kotId: number) => `/kots/${kotId}/activity`,
  markKotAutoPrinted: (kotId: number) => `/kots/${kotId}/mark-auto-printed`,
  releaseKotAutoPrinted: (kotId: number) =>
    `/kots/${kotId}/release-auto-printed`,
  requestFallback: (kotId: number, canPrint = true) =>
    `/kots/${kotId}/request-fallback?can_print=${canPrint}`,
};
...
export const ModifierApis = {
  listGroups: (restaurantId: number) =>
    `/modifiers/groups?restaurant_id=${restaurantId}`,
...
export const DiscountApis = {
  listDiscountsForRestaurant: (restaurantId: number) =>
  ...
  getDiscountByCode: (restaurantId: number, code: string) =>
    `/discounts/restaurant/${restaurantId}/code/${code}`,
};

export const ReservationApis = {
  // Aligning with Flutter: Use /orders endpoints for all reservation actions
  listReservations: (restaurantId: number) =>
    `/orders?restaurant_id=${restaurantId}&channel=reservation`,
...
export const NotificationApis = {
  ...
  forOrder: (
    orderId: number,
    {
      ...
      skip = 0,
      limit = 100,
    }: { restaurantId?: number; skip?: number; limit?: number } = {},
  ) => {
    const params = new URLSearchParams({
      skip: skip.toString(),
      limit: limit.toString(),
...
export const DrawerSessionApis = {
  ...
  controls: ({ restaurantId }: { restaurantId: number }) => {
    const params = new URLSearchParams({
      restaurant_id: restaurantId.toString(),
    });
    return `/drawer-sessions/controls?${params.toString()}`;
  ...
  cashControlSummary: ({
    ...
    return `/drawer-sessions/cash-control-summary?${params.toString()}`;
  },
  suggestion: ({
    restaurantId,
    businessDate,
  ...
  expectedBreakdown: (sessionId: number) =>
    `/drawer-sessions/${sessionId}/expected-breakdown`,
  closingCount: (sessionId: number) =>
    `/drawer-sessions/${sessionId}/closing-count`,
  settlementDecision: (sessionId: number) =>
    `/drawer-sessions/${sessionId}/settlement-decision`,
...
export const AttendanceApis = {
  ...
  schedules: (staffId?: number) =>
    staffId
      ? "/attendance/schedules?staff_id=" + staffId
  ...
  listDeviceMappings: (deviceId?: number) =>
    deviceId
      ? `/attendance/device-mappings?device_id=${deviceId}`
...
export const TransactionsApis = {
  list: ({
    ...
    if (paymentUserId)
      params.append("payment_user_id", paymentUserId.toString());
